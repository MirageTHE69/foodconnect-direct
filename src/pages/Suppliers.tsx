...
export default function BrowseSuppliers() {
  ...
  useEffect(() => { fetchSuppliers(); }, [searchQuery, locationFilter, currentPage]);

  const fetchLocations = async () => {
    const { data } = await supabase.from('supplier_profiles').select('state').eq('verification_status', 'verified');
    const uniqueLocations = new Set<string>();
    data?.forEach(s => { if (s.state) uniqueLocations.add(s.state); });
  ...
  const fetchSuppliers = async () => {
    setLoading(true);
    try {
      let query = supabase.from('supplier_profiles').select('*', { count: 'exact' }).eq('verification_status', 'verified');
      if (searchQuery) query = query.ilike('company_name', `%${searchQuery}%`);
      if (locationFilter && locationFilter !== 'all') query = query.eq('state', locationFilter);
      ...
      const { data: productCounts } = await supabase.from('products').select('supplier_id').in('supplier_id', supplierIds).eq('status', 'approved');

      const countMap = new Map<string, number>();
      productCounts?.forEach(p => { countMap.set(p.supplier_id, (countMap.get(p.supplier_id) || 0) + 1); });

      setSuppliers((data || []).map(s => ({ ...s, product_count: countMap.get(s.id) || 0 })));
  ...
  return (
    <PublicPageLayout>
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Connect with verified food suppliers across India</p>
      </div>
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={locationFilter} onValueChange={(v) => { setLocationFilter(v); setCurrentPage(1); }}>
              <SelectTrigger className="w-full md:w-[180px]"><SelectValue placeholder="All Locations" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Locations</SelectItem>
                {locations.map(loc => (<SelectItem key={loc} value={loc}>{loc}</SelectItem>))}
              </SelectContent>
            </Select>
            <Button type="submit"><Filter className="h-4 w-4 mr-2" />Apply</Button>
          </form>
        </CardContent>
      </Card>
      ...
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-muted-foreground">Showing {suppliers.length} of {totalCount} suppliers</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-muted-foreground max-w-md">Try adjusting your search or filter criteria</p>
        </div>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
